...
export const EzGroupbyTreeMixin = (superclass) => class extends superclass {  
     ...
     * @param func        The datasets.aggregate function to use on each grouping.  Default is aggFunction()
     * @param pathFunc    The path datasets.aggregate function to use on each grouping.  Default is pathAgFunction()
     * 
     ...
     *          [{"field": "company", "datasets.aggregate": "sum(revenue)", "chart": "pie"},
     *           {"field": "date_trunc(month,startdate)", "datasets.aggregate": "sum(revenue)", "chart": "line"},
     *           {"field": "division", "datasets.aggregate": "sum(revenue)", "chart": "bar"},
     *           {"field": "gender", "datasets.aggregate": "sum(revenue)", "chart": "pie"},
     *           {"field": "eyeColor", "datasets.aggregate": "sum(revenue)", "chart": "bar"},
     *           {"field": "age", "datasets.aggregate": "sum(revenue)", "chart": "pie"}]
     * 
     * 
     ...
     * @author Martin Israelsen <martin.israelsen@gmail.com>
     *    Parses the datasets.aggregate string to pull out the field that needs to be datasets.aggregated on.
     *    Example:  sum(revenue)  will return the string "revenue"
     *           
     * @param str        The datasets.aggregate field in the form of agg(field) 
     * 
     * @return The the field to be datasets.aggregated on
     */
    parseAggStr(str) {
        let match = str.match(/\((.*)\)/);
        return match[1];
     ...
     * @author Martin Israelsen <martin.israelsen@gmail.com>
     *    This function is called for each node of the grouping tree.  It datasets.aggregates the values for this grouping
     *    based on the datasets.aggregate defined (i.e. sum, max, min etc) and based on the field being agregated on (i.e. sum(revenue)).    
     *           
            ...
            if (typeof group.datasets[j] != 'undefined') {
                ...
                switch(true){
                    ...
                    case /^boxplot/.test(str) : 
                        ...
                        break;
                    default: 
                        console.log("Unknown datasets.aggregate Function "+str)
                }
            }  
        }   
        return returnArray; 
